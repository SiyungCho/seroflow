...
This module provides implementations for writing DataFrames to CSV files. It defines two loader classes:
    - CSVLoader: Writes a single DataFrame to a CSV file.
    - MultiCSVLoader: Handles writing multiple DataFrames to CSV files.
These classes extend the FileLoader class and implement CSV-specific functionality, such as mapping the file
existence mode to the appropriate file mode for writing.
"""
import os
...
class CSVLoader(FileLoader):
    ...
    A concrete loader for writing a DataFrame to a CSV file. This class extends FileLoader and provides methods 
    to convert DataFrames into CSV format, determine the target file path, and handle file modes according to the 
    specified existence parameter.
    """
...
class MultiCSVLoader(CSVLoader):
    ...
    A concrete loader for writing multiple DataFrames to CSV files. This class extends CSVLoader and provides methods
    to write multiple DataFrames to CSV files in the specified target directory.
    """
    def __init__(self,
                 target,
                 dataframes=None,
