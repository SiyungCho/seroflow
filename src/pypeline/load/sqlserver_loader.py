...
class SQLServerLoader(Loader):
    ...
    Supports specifying multiple target table names, uses an engine to connect to the database.
    ...
    def __init__(self,
        ...
        Args:
            target (str or list): 
                The target table name or list of table names where DataFrames will be loaded.
                If a single string is provided, it will be converted to a list.
            engine: 
                The database engine instance used for connecting to SQL Server.
                (the underlying SQLAlchemy engine).
            step_name (str, optional): 
                The name of this loader step. Defaults to "SQLServerLoader".
            dataframes (list or dict, optional): 
                The DataFrames to load. Defaults to an empty list.
            exists (str, optional): 
                Behavior when a table already exists; 
                expected values are 'append', 'fail', or 'replace'. Defaults to "append".
            **kwargs: Additional keyword arguments to pass to pandas.DataFrame.to_sql.

        Note:
            No explicit type check is performed on the engine;
        """
    ...
    def func(self, context):
        ...
        and writes each DataFrame to a SQL Server table.
        The target table names are taken from the self.target list, 
        and the full schema is constructed using the engine's database and schema attributes.
    ...
    def __to_sql(self, df, target, schema, engine, kwargs):
        ...
        Uses pandas.DataFrame.to_sql to write the DataFrame to the specified table.
        ...
        Args:
            ...
            schema (str): 
                The full schema (combination of database and schema) where the table resides.
            engine: 
                The SQLAlchemy engine used for the database connection.
            kwargs (dict): 
                Additional keyword arguments for pandas.DataFrame.to_sql.
        """
        df.to_sql(target,
                  con=engine,
