...
These classes extend the FileLoader class and implement Excel-specific functionality, including determining the
target file path, handling file modes based on the 'exists' parameter, and managing Excel writing engines.
"""
import os
import pandas as pd
...
class ExcelLoader(FileLoader):
    ...
    A concrete loader for writing a DataFrame to an Excel file. This class extends FileLoader and provides methods
    to convert DataFrames into Excel format, determine the target file path, and handle file modes according to the
...
class MultiExcelLoader(ExcelLoader):
    ...
    A concrete loader for writing multiple DataFrames to multiple Excel files. This class extends ExcelLoader and
    provides methods to write multiple DataFrames to Excel files in the specified target directory.
    """
    def __init__(self,
                 target,
                 dataframes=None,
