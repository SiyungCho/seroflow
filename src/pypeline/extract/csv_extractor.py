...
It defines two classes:
    - CSVExtractor: Extracts a DataFrame from a single CSV file. It supports both standard reading and chunked reading
      when a chunk_size is specified.
    - MultiCSVExtractor: Extracts DataFrames from multiple CSV files located in a given source directory.
Both classes extend from their respective base extractor classes and leverage pandas' read_csv() functionality to 
read data. Additionally, CSVExtractor provides a method to determine the maximum number of rows in a CSV file, which 
is useful for chunking operations.
"""
...
class CSVExtractor(FileExtractor):
    ...
    A concrete extractor for reading data from CSV files. This class extends FileExtractor and provides methods
    to read an entire CSV file or to read it in chunks when a chunk_size is specified. The extracted DataFrame is 
    added to the context under the file name.
    """
    def __init__(self,
                 source,
...
class MultiCSVExtractor(MultiFileExtractor):
    ...
    A concrete extractor for reading multiple CSV files from a specified source directory. This class extends 
    MultiFileExtractor and leverages CSVExtractor to extract DataFrames from each CSV file found in the directory.
    """
