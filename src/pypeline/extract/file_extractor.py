...
It provides two main classes:
    - FileExtractor: An abstract base class that extends Extractor for reading a single file.
      It validates the source file, extracts the file name (without extension), and defines an interface
      for reading the file either entirely or in chunks.
    - MultiFileExtractor: A concrete extractor that extends MultiExtractor to handle multiple files.
      It gathers files from a specified source directory based on a given extension type and creates multiple
      extractor instances to process each file individually.
"""
...
class FileExtractor(Extractor):
    ...
    This class extends the Extractor class and validates the source file, extracts the file name,
    and defines abstract methods for reading the file fully, reading the file in chunks, and obtaining
...
class MultiFileExtractor(MultiExtractor):
    ...
    This class extends the MultiExtractor and gathers file paths and file names from the source directory based on
    a specified file extension type. It then creates and adds individual extractor instances for each file.
    """
    def __init__(self,
                 source,
                 type,
