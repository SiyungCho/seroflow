...
This module provides a concrete implementation for extracting data from any ODBC‑accessible data source using raw SQL.
It defines a single class:

    - ODBCExtractor: Extracts data from a table or view over a pyodbc.Connection, supporting both full-table reads and chunked reads via OFFSET/FETCH.

Each extraction returns a pandas DataFrame and integrates with the framework’s Extractor interface for pipeline orchestration.
"""
import pandas as pd
...
class ODBCExtractor(Extractor):
    ...
    def _default_order_by(self) -> str:
        """
        Infer a default ORDER BY column by selecting a single row and returning its first column name.
